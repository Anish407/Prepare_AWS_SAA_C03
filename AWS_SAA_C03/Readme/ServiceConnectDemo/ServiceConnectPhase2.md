...
![ECS Fargate Service Connect end-to-end TLS architecture](./images/service-connect-tls-architecture.png)

```mermaid
flowchart TB
    Viewer["Client / browser"]
    R53["Route 53<br/>api.example.com"]
    CF["CloudFront distribution<br/>Viewer HTTPS<br/>Origin policy: HTTPS only"]
    CFACM["ACM public certificate<br/>us-east-1<br/>(only for a custom CloudFront domain)"]

    subgraph Region["AWS Region"]
        OriginDNS["Route 53 origin record<br/>api-origin.example.com"]
        ALBACM["Regional ACM public certificate<br/>for api-origin.example.com"]

        subgraph VPC["serviceconnectdemo-vpc — 10.0.0.0/16"]
            IGW["Internet Gateway"]

            subgraph Public["Public subnets in two Availability Zones"]
                ALB["Internet-facing Application Load Balancer<br/>HTTPS listener :443<br/>TLS 1.3-capable policy<br/>ALB security group"]
                TG["Api1 target group<br/>IP targets · HTTPS :8080<br/>HTTPS /health check :8080"]
            end

            subgraph Private["Private subnets in two Availability Zones — no public IPs"]
                subgraph T1["Api1 Fargate task · Api1 security group"]
                    P1["Service Connect proxy<br/>TLS ingress :8080<br/>ingressPortOverride unset"]
                    A1["Api1 app<br/>HTTP :8080<br/>Api2 URL: http://api2:8080"]
                end

                subgraph T2["Api2 Fargate task · Api2 security group"]
                    P2["Service Connect proxy<br/>TLS ingress :8080"]
                    A2["Api2 app<br/>HTTP :8080<br/>Api3 URL: http://api3:8080"]
                end

                subgraph T3["Api3 Fargate task · Api3 security group"]
                    P3["Service Connect proxy<br/>TLS ingress :8080"]
                    A3["Api3 app<br/>HTTP :8080"]
                end

                subgraph Endpoints["VPC endpoints"]
                    ECRAPI["Interface: ecr.api"]
                    ECRDKR["Interface: ecr.dkr"]
                    LOGS["Interface: logs"]
                    SM["Interface: secretsmanager"]
                    KMS["Interface: kms<br/>(customer managed key)"]
                    S3["Gateway: s3"]
                    EPSG["Endpoint security group<br/>inbound TCP 443 from task SGs"]
                end
            end
        end

        ECR["Three private ECR repositories"]
        CW["CloudWatch Logs"]
        Secrets["Secrets Manager<br/>Service Connect TLS material"]
        KMSService["AWS KMS"]
        PCA["AWS Private CA<br/>AmazonECSManaged=true"]
        Namespace["Service Connect namespace<br/>serviceconnectdemo.local<br/>Cloud Map HTTP namespace"]
        Cluster["ECS Fargate cluster<br/>three ECS services"]
        ExecRole["Task execution role<br/>AmazonECSTaskExecutionRolePolicy"]
        InfraRole["ECS infrastructure role<br/>Service Connect TLS managed policy"]
    end

    Viewer -->|"DNS"| R53
    R53 -->|"HTTPS"| CF
    CFACM -.-> CF
    CF -->|"HTTPS origin request"| OriginDNS
    OriginDNS --> IGW
    IGW --> ALB
    ALBACM -.-> ALB
    ALB --> TG
    TG -->|"HTTPS / TLS 1.3 :8080"| P1

    P1 -->|"task-local loopback HTTP"| A1
    A1 -->|"HTTP to local client proxy"| P1
    P1 -->|"Service Connect TLS 1.3 :8080"| P2
    P2 -->|"task-local loopback HTTP"| A2
    A2 -->|"HTTP to local client proxy"| P2
    P2 -->|"Service Connect TLS 1.3 :8080"| P3
    P3 -->|"task-local loopback HTTP"| A3
    ...
    Namespace -.-> P2
    Namespace -.-> P3
    PCA -.->|"issues and rotates proxy certificates"| InfraRole
    InfraRole -.-> P1
    InfraRole -.-> P2
    InfraRole -.-> P3
    ExecRole -.->|"image pulls and log configuration"| Cluster
...
The `ecr.api`, `ecr.dkr`, `logs`, `secretsmanager`, and optional `kms` endpoints are interface endpoints with private DNS enabled and the endpoint security group attached. The `s3` endpoint is a gateway endpoint associated with the private route tables. ECR image pulls require both ECR interface endpoints and the S3 gateway endpoint. AWS Private CA is used by the ECS control plane through the infrastructure role, so the tasks do not need an `acm-pca` VPC endpoint.
