...
```text
The trust policy uses ```ecs.amazonaws.com```, unlike an ECS task role or task execution role, which uses ```ecs-tasks.amazonaws.com```.

This is because the infrastructure role is assumed by the Amazon ECS service control plane to manage Service Connect TLS resources; it is not assumed by the running ECS tasks
```
...
```
<img width="902" height="273" alt="image" src="https://github.com/user-attachments/assets/d7f3bb5c-845a-4ab4-80fe-f6052404a317" />
